...
function SignupForm() {
  ...
  async function handleEmailSignUp(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setLocalError(msg.includes("email-already-in-use") ? "An account with this email already exists" : msg);
      setLoading(false);
    }
  }
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      setLocalError(msg.includes("popup-closed") ? "Sign in cancelled" : msg);
  ...
  return (
    <div className="w-full max-w-sm">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">Create your account</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Start screening smarter with HireLens AI</p>
      </div>
      ...
      {success && (
        <div className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700 dark:border-emerald-900/50 dark:bg-emerald-950/30 dark:text-emerald-300">
          <CheckCircle2 className="mb-1 inline h-4 w-4" /> {decodeURIComponent(success)}
        </div>
      )}
      ...
      <form onSubmit={handleEmailSignUp} className="space-y-4">
        <div>
          ...
          <input id="fullName" name="fullName" type="text" autoComplete="name" required placeholder="John Smith"
      ...
      <button onClick={handleGoogleSignIn} disabled={googleLoading}
        className="focus-ring flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm transition-all hover:bg-slate-50 active:scale-[0.98] disabled:opacity-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">
